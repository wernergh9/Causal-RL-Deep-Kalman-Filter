...
def eval_policy(policy, logger, monitor, video_recorder, seed, env_cfg, step, max_episode_steps, nb_eval_episodes, record=False, nb_recorded_episodes=20):
    ...
    for episode in tqdm(range(0, nb_eval_episodes), desc ="Model Evaluation"):
        ...
        while not done and (episode_step < max_episode_steps):
            ...
            if episode < nb_recorded_episodes:
                video_recorder.record(eval_env, episode=episode, returns=episode_reward, step=episode_step)
    ...
    logger.log('eval/episode_reward_avg', average_episode_reward, step)
    logger.log('eval/episode_reward_std', np.array(list_episode_reward).std(), step)
